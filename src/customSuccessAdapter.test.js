...
let originalRequest;
beforeEach(() => {
  originalRequest = { attemptNumber: 1, headers: {}, method: 'GET', url: API_BASE_URL };
  apiResponse = { api: 'res' };
...
describe('httpClient', () => {
  describe('customSuccessAdapter', () => {
    ...
    test('should be called with the successAdapter response', () => {
      const customSuccessAdapter = jest.fn((res) => Promise.resolve({ ...res, aLittle: 'extra' }));
      const apiClient = clientBuilder({ customSuccessAdapter });
      return apiClient
        .get(API_BASE_URL)
        .then(res => {
          expect(customSuccessAdapter).toHaveBeenCalledWith(apiResponse, originalRequest);
